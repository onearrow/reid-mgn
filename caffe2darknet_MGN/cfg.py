...
def print_cfg_nicely(blocks):
    ...
    for block in blocks:
        ...
        if block['type'] == 'net':
            ...
            out_heights.append(prev_height)
            out_filters.append(prev_filters)
        elif block['type'] == 'slice':
            print('%5d %-6s                                     ->     %3d' % (ind, 'slice', prev_filters))
            out_widths.append(prev_width)
            out_heights.append(prev_height)
            out_filters.append(prev_filters)
            ...
            if len(layers) == 1:
                ...
                print('%5d %-6s %d %d %d %d %d %d %d %d' % (ind, 'route', layers[0], layers[1], layers[2], layers[3], layers[4], layers[5], layers[6], layers[7]))
                prev_width = out_widths[layers[0]]
                prev_height = out_heights[layers[0]]
                out0 = out_filters[layers[0]]
                out1 = out_filters[layers[1]]
                out2 = out_filters[layers[2]]
                out3 = out_filters[layers[3]]
                out4 = out_filters[layers[4]]
                out5 = out_filters[layers[5]]
                out6 = out_filters[layers[6]]
                # print(len(out_filters))
                # print(layers[7])
                out7 = out_filters[layers[7]]
                prev_filters = out0 + out1 + out2 + out3 + out4 + out5 + out6 + out7
            out_widths.append(prev_width)
            out_heights.append(prev_height)
            out_filters.append(prev_filters)
