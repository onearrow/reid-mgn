...
def Test(TEST_PATH):
    ...
    for id_name in id_list:
        ...
        for image_name in image_list:
            ...
            if image_id not in id_allocation_list:
                id_allocation_list.append(image_id)
        ID_allocation_list.extend(id_allocation_list)
        true_id_name  = image_id_list[0]
        true_id_num   = float(image_id_list.count(true_id_name))
    ...
    FEATURE_allocation_error = false_id_sum # 3
    
    ID_allocation_list.sort()
    x_value = []
    y_value = []
    for true_id in ID_allocation_list:
        if true_id not in x_value:
            num = ID_allocation_list.count(true_id)
            if num>1:
                x_value.append(true_id)
                y_value.append(ID_allocation_list.count(true_id))
    
    print len(x_value)
    print max(y_value)
    fig, ax1 = plt.subplots() # 使用subplots()创建窗口
    ax1.bar(x_value, y_value, 0.5)
    ax1.set_xlabel("Real ID number") 
    ax1.set_ylabel("Number of ID allocation")
    plt.title('Statistics of ID allocation, Market-1501, Threshold2=0.4')
    plt.savefig("ID_allocation_M_Threshold2=0.4.jpg")
    
    print "ID acc: %f" %ID_acc
    print "Class acc: %f" %CLASS_acc
    print "FEATURE allocation error: %d" %FEATURE_allocation_error
    print "ID allocation error: %f" %(len(x_value)/float(GROUDTRUTH_ID_NUM))
...
if __name__ == "__main__":
    ...
    GROUDTRUTH_ID_NUM    = 750 # 750 212 7
    DATA_SET = "M"
    ...
    MODEL_PATH       = "model/MGN_12_27_M.pt"
