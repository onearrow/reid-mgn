...
#include "slice_layer.h" // 2018.11.22-lwp
...
LAYER_TYPE string_to_layer_type(char * type)
{
    ...
    if (strcmp(type, "[slice]")==0) return SLICE; // 2018.11.22-lwp
...
maxpool_layer parse_maxpool(list *options, size_params params)
{
    // int stride = option_find_int(options, "stride",1);
    // int size = option_find_int(options, "size",1);
    int size_w = option_find_int_quiet(options, "size_w", 1);
    int size_h = option_find_int_quiet(options, "size_h", 1);
    int stride_w = option_find_int_quiet(options, "stride_w", 1);
    int stride_h = option_find_int_quiet(options, "stride_h", 1);
    int padding = option_find_int_quiet(options, "padding", 0);
    ...
    maxpool_layer layer = make_maxpool_layer(batch,h,w,c,size_w,size_h,stride_w,stride_h,padding);
    return layer;
}
